...
export function createSocket(auth: Auth, ignoreCertificates: boolean): Promise<any> {
  ...
  debug("hass:init")("Initializing WebSocket connection to Home Assistant");
  debug("hass:init")(url);

  function connect(triesLeft: number, promResolve: (socket: any) => void, promReject: (err: number) => void) {
    debug("hass:connect")(`Connecting to Home Assistant... Tries left: ${triesLeft}`);
    ...
    const closeOrError = (errorText?: string) => {
      if (errorText) {
        debug("hass:connect")(`WebSocket Connection to Home Assistant closed with an error: ${errorText}`);
      }
      if (invalidAuth) {
    ...
    const handleMessage = (event: { data: any; type: string; target: WebSocket }) => {
      ...
      debug("hass:connect")(`Received: ${message.type}`, message);
      ...
      switch (message.type) {
        ...
        default:
          ...
          if (message.type !== MSG_TYPE_AUTH_REQUIRED) {
            debug("hass:connect")("Unhandled message", message);
          }
      }
    };
